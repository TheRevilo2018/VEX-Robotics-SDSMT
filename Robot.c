...
task autostack()
{
	...
	int coneHeight[9] = { 70, 220, 460, 460, 755, 940, 1130, 1370, 1700}; //30, 190, 460, 755, 940, 1130, 1370, 1610
	int SwingHigh = 4000;
	...
	while((SensorValue[LeftPot] < coneHeight[numCones] -50) || (SensorValue[LeftPot] > coneHeight[numCones] + 50))
	{
		...
		{
			motor[LeftLift] = (coneHeight[numCones] - CurrentL)*40 ;
			motor[RightLift] = (coneHeight[numCones] - CurrentL)*40 ;
			wait1Msec(20);
		}
