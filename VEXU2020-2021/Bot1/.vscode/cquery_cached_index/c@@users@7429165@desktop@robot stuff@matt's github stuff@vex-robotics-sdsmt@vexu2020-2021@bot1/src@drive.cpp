...
FourWheelDrive::FourWheelDrive(vector<Motor> & right, vector<Motor> & left)
{
    ...
    lcd::set_text(3, "drive initialized: " + to_string(rightPointer->size()) +
        " " + to_string(leftPointer->size()));
}
...
void FourWheelDrive::setZeroPosition()
{
    ...
    {
        pros::lcd::set_text(i, "for loop: " + to_string((*leftMotors)[i].get_position()));
...
void FourWheelDrive::driveDist(double target, DIRECTION direction, double maxSpeed)
{
    ...
    if (maxSpeed > 10)
    {
        ...
        while (averagePos < startDistance && stopCount < STOP_AMOUNT)
        {
            ...
            pros::lcd::set_text(4, "averagePos: " + to_string((*leftMotors)[0].get_position()) + " " +
                to_string((*rightMotors)[1].get_position()));
        ...
        //pros::lcd::set_text(5, "if: " + to_string(count));
