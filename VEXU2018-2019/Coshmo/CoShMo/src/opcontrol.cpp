...
void opcontrol()
{
	...
	liftMotorLeft.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
	liftMotorRight.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD); // aaaa
		...
		if(master.get_digital(pros::E_CONTROLLER_DIGITAL_RIGHT))
    {
        if(debounceButtonRIGHT <= 0)
				{
						//highScore(leftWheelMotorVector, rightWheelMotorVector, liftMotors);
    ...
    else
    {
      ...
      else
      {
		...
		liftMotorRight.move_absolute(liftPos, 127);
