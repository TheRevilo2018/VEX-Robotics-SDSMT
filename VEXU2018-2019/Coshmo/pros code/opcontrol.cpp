...
void opcontrol()
{
  ...
  pros::Controller partner(pros::E_CONTROLLER_PARTNER);
	...
	pros::Motor rightWheel1(11);
	pros::Motor rightWheel2(12);
	pros::Motor rightWheel3(13);
	...
	while (true)
	{
      ...
      {
             leftMotorPercent += fabs(master.get_analog(ANALOG_RIGHT_X) / 4.0);
             rightMotorPercent -= fabs(master.get_analog(ANALOG_RIGHT_X) / 4.0);
      }
      else
      {
          leftMotorPercent -= fabs(master.get_analog(ANALOG_RIGHT_X) / 4.0);
